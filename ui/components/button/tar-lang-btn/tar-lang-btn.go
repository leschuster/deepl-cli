...
// Provides the button to head over to the "select target language" page
type Model struct {
...
func InitialModel(ctx *context.ProgramContext) Model {
	return Model{
		ctx: ctx,
		btn: button.InitialModel(ctx, "Target Language", "select"),
	}
}
...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case key.Matches(msg, m.ctx.Keys.Select):
			return m, com.TarLangBtnSelectedCmd()
		}
	}
